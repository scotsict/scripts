...
updateDist() {
    ...
    response=$(curl -s -H "Authorization: Bearer ${bearerToken}" --request GET \
     --url $url/JSSResource/policies \
     --header 'accept: application/json'\
     --header 'content-type: application/json' \
    )

    #  --url https://yourserver.jamfcloud.com/JSSResource/distributionpoints \
    #  --header 'accept: application/json'
    echo $response
    # id="1"
    # curl -k -s -u "$username" -X "PUT" "$url/JSSResource/policies/id/$id/subset/PackageConfiguration" \ -H "Content-Type: application/xml" \ -H "Accept: application/xml" \ -d "<policy><package_configuration><distribution_point>default</distribution_point></package_configuration></policy>" exit 0
